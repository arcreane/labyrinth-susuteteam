...
public class MazeController implements Initializable {
    ...
    @FXML
    private TextField ColumnNumberText;

    @FXML
    private TextField RowNumberText;

    @FXML
    private Button TestingButton;

    @FXML
    private AnchorPane rootUIGridMenu;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for (int i = 0; i <totalColumn; i++) {
            for (int j = 0; j < totalRow; j++) {
                gridPane.add(new Label("" + (i+j)), i, j); //TODO Remplacer le label par les Nodes 🙂
            }
        }
        rootUIGridMenu.getChildren().addAll(gridPane);
    ...
    public static void init(){
        A_Maze_Ing(10);
        GridPane ActualGridView = new GridPane();

        ActualGridView.addRow(10);
        ActualGridView.addColumn(10);
    ...
    public static void A_Maze_Ing(int table_length) {
        ...
        for(int x = 0; x < table.length; x++){
            ...
            for (int y = 0; y < table[x].length; y++) {
                Label teotsm = new Label("" + table[x][y]);
                if (Objects.equals(table[x][y], "O")){
                     System.out.print(ANSI_RED + table[x][y] + ANSI_RESET + "  ");
                   // .setStyle("-fx-background-color:black;", x, y);
                    //ActualGridView.add(teotsm, x, y);
                }
                else{
    ...
    public static void mazeGeneration(String[][] maze, String[] coord_entrance, String[] coord_exit) {
        ...
        while (coord_x != coords_exit_x || coord_y != Integer.parseInt(coord_exit[1])) {
            ...
            if (magic_random_number2 <= the_probability_to_go_to_exit) {
                System.out.println("magic_random_number2 = " + magic_random_number2);
    ...
    public static void mazeGeneration2(String[][] maze){
        int random_path1 = 0;
        int random_path2 = 0;
        for(int i = 1; i < maze.length*6; i++){
            for(int j = 1; j < maze.length*6; j++){
                random_path1 = (int) ((Math.random()*maze.length-1)+1);
                random_path2 = (int) ((Math.random()*maze.length-1)+1);
                maze[random_path1][random_path2] = "O";
            }
        }
    }
    ...
    public static void SolutionFinish(){
